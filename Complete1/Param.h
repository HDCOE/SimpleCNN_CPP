...
struct cache
{
	tensor_t<float> dA; //output
	std::vector<tensor_t<float>> dW;
	tensor_t<float> db;
	...
	{
			//initialize Weight
		tensor_t<float> dW_t(dWsize.x, dWsize.y, dWsize.z);
		
		for (int i = 0; i < Nfilter; ++i)
		{
			dW_t = create_tensor(dW_t.size.x, dW_t.size.y, dW_t.size.z,0);
