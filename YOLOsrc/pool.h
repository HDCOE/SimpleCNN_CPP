                    ...
                    for(n = 0; n < l.size; ++n){
                        for(m = 0; m < l.size; ++m){
                            ...
                            int valid = (cur_h >= 0 && cur_h < l.h &&
                                         cur_w >= 0 && cur_w < l.w);
                            float val = (valid != 0) ? (float)net.input[index] : -FLT_MAX;
...
layer parse_maxpool(int size, int stride, int padding, size_params params)
{
    int batch,h,w,c;
    h = params.h;
    w = params.w;
    ...
    layer maxpool_layer = make_maxpool_layer(batch,h,w,c,size,stride,size -1 ); // pad is size - 1
